...
namespace bitbox
{
    class SpaceInvadersInvader
    {
        ...
        public SpaceInvadersInvader(Vector2i position, int gridX, int gridY)
        {
            ...
            UpdateLevel();
            invaderRect.Position = new Vector2f(invaderRect.Size.X*2 + (SpaceInvadersGame.windowSize.X / invaderRect.Size.X * 3) * invaderPosition.X, newLevelPosition); // Initial position

            //SpaceInvadersGame.InvaderTexture(ref invaderRect, animation);
        }
